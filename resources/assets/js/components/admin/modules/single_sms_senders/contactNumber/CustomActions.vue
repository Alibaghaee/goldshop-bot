<template>
    <div class="flex justify-center -mx-1">
        <input
            ...
            type="checkbox" name="contactNumber_ids[]"
...
export default {
    ...
    methods: {
        onInit() {
            ...
            EventBus.$on("contactNumbersSelectionCond", (cond) => {


                if (cond === 'selected_all') {

                    EventBus.$emit('setContactNumberId', this.rowData.id);
                    // this.checkedCond = 'checked';
                } else if (cond === 'unselected_all') {

                    EventBus.$emit('unsetContactNumberId', this.rowData.id);
                    // this.checkedCond = 'unchecked';
                }
            });
            EventBus.$on("contactNumbersSelectionList", (list) => {
        ...
        onClick(id, event) {
            if (event.target.checked) {
                ...
                EventBus.$emit('setContactNumberId', id);
            } else {

                /// unset id
                EventBus.$emit('unsetContactNumberId', id);
            }
        }
    },
