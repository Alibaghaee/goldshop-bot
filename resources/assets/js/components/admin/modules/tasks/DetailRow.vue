  ...
  <div @click="onClick" class="flex">
    <div class="w-full md:w-1/2">
      <div>
        ...
        <span>{{rowData.creator_fullname}}</span>
      </div>
      <div>
        <label class="font-bold">دریافت کننده:</label>
        <span>{{rowData.receiver_fullname}}</span>
      </div>
      <div>
      ...
      <div>
        ...
        <span class="inline-block ltr">{{rowData.start_date}}</span>
      </div>
      <div>
        <label class="font-bold">مهلت:</label>
        <span class="inline-block ltr">{{rowData.deadline}}</span>
      </div>
      <div>
        <label class="font-bold">وضعیت گزارش:</label>
        <span>{{rowData.report_status}}</span>
      </div>
      <div>
        <label class="font-bold">زمان گزارش:</label>
        <span class="inline-block ltr">{{rowData.reported_at}}</span>
      </div>
       <div>
        <label class="font-bold">گزارش فعالیت: </label>
        <span>{{rowData.report_description}}</span>
      </div>
      <div>
    ...
    <div class="w-full md:w-1/2 p-4 rounded bg-grey-light">
      <div class="font-bold mb-2">
        ...
        <span class="font-medium text-xs opacity-75">(شناسه {{rowData.id}}#) </span>:
      </div>
      <div class="flex flex-row flex-wrap justify-between">
        ...
        <div v-if="rowData.report_status">
          <label class="font-bold">وضعیت گزارش:</label>
          <span>{{rowData.report_status}}</span>
        </div>
        <div v-if="rowData.reported_at">
          <label class="font-bold">زمان گزارش:</label>
          <span class="inline-block ltr">{{rowData.reported_at}}</span>
        </div>
         <div v-if="rowData.report_description">
          <label class="font-bold">گزارش فعالیت: </label>
          <span>{{rowData.report_description}}</span>
        </div>
        <div v-if="rowData.status">
          <label class="font-bold">وضعیت انجام کار:</label>
          <span>{{rowData.status}}</span>
        </div> 
      </div>
      <div v-for="refer in rowData.refers" class="border-grey border-t mt-4 pt-4">
