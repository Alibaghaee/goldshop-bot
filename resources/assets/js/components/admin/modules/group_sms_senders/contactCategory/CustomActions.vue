<template>
    <div class="flex justify-center -mx-1">
        <input
            ...
            type="checkbox" name="contactCategory_ids[]"
...
export default {
    ...
    methods: {
        onInit() {
            ...
            EventBus.$on("contactCategoriesSelectionCond", (cond) => {


                if (cond === 'selected_all') {

                    EventBus.$emit('setContactCategoryId', this.rowData.id);
                    // this.checkedCond = 'checked';
                } else if (cond === 'unselected_all') {

                    EventBus.$emit('unsetContactCategoryId', this.rowData.id);
                    // this.checkedCond = 'unchecked';
                }
            });
            EventBus.$on("contactCategoriesSelectionList", (list) => {
        ...
        onClick(id, event) {
            if (event.target.checked) {
                ...
                EventBus.$emit('setContactCategoryId', id);
            } else {

                /// unset id
                EventBus.$emit('unsetContactCategoryId', id);
            }
        }
    },
