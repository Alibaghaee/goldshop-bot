        ...
        <div class="flex h-full flex-wrap">
            <form class="w-1/2 flex flex-col flex-wrap justify-between m-8  w-full   rounded border-2 p-2"
                ...
                <div class="flex flex-col lg:flex-row">
                    ...
                    <div class="mr-5"></div>
                    <div class="text-blue py-2 mr-2 ">تعداد گروه انتخاب شده :
                        <div class="inline text-blue font-bold">
                            {{ list.length }}
                        </div>

                    </div>
                    <div class="rhc-btn rhc-btn-pink m-2 " v-on:click="unselectAllContactCategory">بازنشانی</div>
                    <div class="rhc-btn rhc-btn-blue m-2" v-on:click="selectAllContactCategory">انتخاب صفحه</div>
                </div>
...
export default {
    ...
    data() {
        return {
            endpoint: '/group_sms_senders/group_sms_senders',
            form_data: {},
    ...
    methods: {
        ...
        onFormSubmit() {
            ...
            this.form_data.contactCategory_ids = JSON.stringify(this.list);
        ...
        selectAllContactCategory(event) {

            EventBus.$emit('contactCategoriesSelectionCond', 'selected_all');
        },
        unselectAllContactCategory(event) {

            EventBus.$emit('contactCategoriesSelectionCond', 'unselected_all');
        },
        addSMSCounter() {
    ...
    created() {
        ...
        EventBus.$on("setContactCategoryId", (id) => {

            const index = this.list.indexOf(id);
            if (!(index > -1)) {
                this.list.push(id);
                EventBus.$emit('contactCategoriesSelectionList', this.list);
            }

        });

        EventBus.$on("unsetContactCategoryId", (id) => {
