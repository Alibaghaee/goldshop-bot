...
export default {
    ...
    methods: {
        onInit() {
            ...
            EventBus.$on("driversSelectionCond", (cond) => {
                if (cond === 'selected_all') {

                    EventBus.$emit('setDriverId', this.rowData.id);
                    // this.checkedCond = 'checked';
                } else if (cond === 'unselected_all') {

                    EventBus.$emit('unsetDriverId', this.rowData.id);
                    // this.checkedCond = 'unchecked';
                }
            });
            EventBus.$on("driversSelectionList", (list) => {
        ...
        onClick(id, event) {
            if (event.target.checked) {
                ...
                EventBus.$emit('setDriverId', id);
            } else {

                /// unset id
                EventBus.$emit('unsetDriverId', id);
            }
        }
    },
